...
def testWFO(f1, f2, i):
    cmd = '../wavefront -WFO2OCL -fP ../data/{} -fT ../data/{} -wgs 32 -v -k -1 -tl {}'.format(f1,f2, i)
    #print(cmd)
    p = os.popen(cmd)
    res = p.read()
...
def testWFDD(f1, f2, i):
    cmd = '../wavefront -WFDD2OCL -fP ../data/{} -fT ../data/{} -wgs 32 -v -k -1 -tl {}'.format(f1,f2, i)
    #print(cmd)
    p = os.popen(cmd)
    res = p.read()
    lines = res.splitlines()
...
for i in range(1,20):
    ...
    #testWFDD('CP000051.1.fasta', 'AE002160.2.fasta',i)
    #testWFDD('BA000035.2.fasta', 'BX927147.1.fasta', i)
    testWFDD('NC_005027.1.fasta', 'NC_003997.3.fasta', i)
