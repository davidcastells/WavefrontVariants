...
def test(f1, f2, tl):
    cmd = '../wavefront -pid 1 -WFO2OCL -fP ../data/{} -fT ../data/{}  -v -k -1 -tl {} --local-tile-memory shared -pp 10'.format(f1,f2,  tl)
    #print(cmd)
    ...
    print('{},{},{},{},{},{}'.format(tl,registers, shmem, cmem, time, gcups))
...
print('tl,register,shmem,cmem,time,gcups')

df = pd.read_csv('parallel_workgroup.txt', skiprows=1)

max_wgs = 1024
tl_max= (max_wgs - 1) // 2

for tl in range(tl_max, 0, -1):
    df_tl = df[df['tl'] == tl]
    
            
    if (len(df_tl) == 0):
        test('BA000035.2.fasta', 'BX927147.1.fasta', tl)
...
# LAUNCH WITH : nohup python3 -u test_parallel_workgroup.py > new_parallel_workgroup.txt &
